...
module Info = struct
  ...
  let get_my_domid xs =
    Os_xen.Xs.(immediate xs (fun h -> read h "domid"))

  let register_me xs myname =
    get_my_domid xs >>= fun domid ->
    Os_xen.Xs.(immediate xs (fun h -> write h ("/conduit/" ^ myname) domid))

  let get_domid_for xs name =
    Os_xen.Xs.(immediate xs (fun h -> read h ("/conduit/" ^ name)))
end
...
module Server (C: Mirage_console_lwt.S) = struct
  ...
  let readdir h d =
    ...
    Os_xen.Xs.(directory h d) >>= fun dirs ->
  ...
  let start c =
    ...
    Os_xen.Xs.wait xs
      (fun h ->
         ...
         Os_xen.Xs.read h (sprintf "/conduit/%s" remote_name) >>= fun remote_domid ->
...
module Client (C: Mirage_console_lwt.S) = struct
  ...
  let start c =
    ...
    Info.register_me xs name >>= fun () ->
    Os_xen.Xs.(immediate xs (fun h -> read h (sprintf "/conduit/%s" server_name))) >>= fun remote_domid ->
    let remote_domid = int_of_string remote_domid in
    Os_xen.Xs.(immediate xs (fun h -> write h (sprintf "/conduit/%s/%s/%s" server_name name port) "theport"))
    >>= fun () ->
    Vchan.Port.of_string port
    |> function
